...
> **注（V10·A1/A8）**：`q = 1/φ²` 取自**几何骨架，非实测**；"等距 / 对数周期"这一**性质**严格，但**步长数值完全随 `q`**（`q` 待实测，见总报告 V1）。
...
**等距**（步长恒为 `ln q`；下表按 `q = 1/φ²` 取值）⟹ 这就是 DSI 的"格点"⟹ 任何**耦合到层结构**的探针，
都会看到以 `ln q` 为周期的响应。
...
| 本结构是 `ln r` 上的离散格点 ⟹ 具对数周期签名（**取 `q = 1/φ²`；`q` 待实测，见 V1**）（V10·A1/A8） | **严格（几何层面）** |
...
**本节检验的“族”需要精确定义**（原先笼统写作 “generic C₅ 模型”，见 §15.5 的修订说明）：

| 要素 | 内容 |
| :-- | :-- |
| 结构 | 5 节点环耦合（`S`）+ **逐点多项式**非线性 |
| 参数 | `d, a, c` 取**常数**；临界线 `a = d` |
| **关键特征** | **混合次数**（1 次线性 + 3 次非线性）⟹ **无单一标度维数** |
| **关键缺失** | 族中**没有任何尺度参数 `q`**——无层级、无跨尺度反馈 |
...
- 在 `ln t` 上做 DFT，找周期；
- 目标频率 `f* = 1/|ln q| = 1.039`（**依赖 `q`**；`q` 待实测，V10·A8）；
...
**原先的诊断**写作「`C₅` 是“旋转”对称、不是“标度”对称」。**这句话对，但不是操作性原因**——
更精确的原因在**函数层**：

> **本模型【不齐次】——“线性项按 `λ` 走、立方项按 `λ³` 走”，不存在统一的标度维数。**

直接检验（在模型上拟合 `F(λx) = λ^p·F(x)` 的最优 `p`）：

| 模型 | 最优 `p`（取 `λ=1/φ²`） | 相对残差 | 判定 |
| :-- | :-- | :-- | :-- |
| **本模型**（1 次 + 3 次） | 0.856 | **0.692** | ✗ **不齐次** |
...
1. **无 `q`-协变 ⟹ 无 DSI ⟹ 不可能有对数周期**——这是**结构性**的，不是“试了没找到”；
2. **`q` 根本不在模型里**：问“有没有以 `q` 为周期的振荡”，答案必然是没有；
3. **与 `C₅` 无关**：换任何对称群，**只要混合次数，结论照旧**；
   反过来，**只要齐次，`C₅` 也可以有 DSI**（§十六 正是这样构造的）。

> 补充：旋转对称 `S⁵ = I` 是**指标**上的周期，标度对称 `q^ℤ` 是**尺度**上的周期，两者**互相独立**。
> **决定有没有 DSI 的是函数是否齐次，而不是群的类型。**

**详见**：`generic-family-definition.md`（本目录，读解续篇）。
...
| **该族**（混合次数的 `C₅` 环耦合模型，定义见 §15.1）临界附近含“以 `ln q` 为周期”的对数周期 | **❌ 否定**（谱检验 + 正对照；且**结构性不可能**——该族不齐次） |
| 该族**不齐次** ⟹ 无 `q`-协变 ⟹ **不可能**有 DSI | **严格（解析 + 数值）** |
...
| :-- | :-- |
| **几何** | **已有** DSI（严格，§十三） |
| **动力学（混合次数的非齐次族）** | **没有**（本节否定；且**结构性不可能**） |
...
§十五 得到否定结果，并把诊断**升级**为：该族**混合次数、不齐次** ⟹ 无 `q`-协变 ⟹ 无 DSI
（原"`C₅` 是**旋转**对称、不是**标度**对称"之说**对，但不是操作性原因**，见 §15.5）。
本节把这个诊断**反过来用**，就得到了答案。
...
*v1.6 补注（2026-09-18）：落实 V10 前提审计——§十四 补 A1/A8（`q` 待实测）、§十五 补 A8（目标频率依赖 `q`）、§十六 开头同步升级后的诊断*
*时间：2026-09-17*
