...
- 相生矩阵 $R$ 是循环置换矩阵：$R^5=I$，$\{I,R,R^2,R^3,R^4\}$ 构成 5 阶循环群，且都是正交矩阵；生、克、侮、母子四种关系**全是 $R$ 的幂**。
- 五行图 = 仿射 Dynkin 图 $\tilde A_4$，Cartan 矩阵 $A=2I-\mathrm{Adj}(C_5)$：$\det(A)=0$、秩 4、半正定（**仿射型**），零根 $=(1,1,1,1,1)$；非零特征根 $=2+\varphi$ 与 $2-\tfrac1\varphi$（黄金比 $\varphi$ 自动出现）。
- 生克加权 $M(a,b)=2I-a\,\mathrm{Adj}(生)-b\,\mathrm{Adj}(克)$：$a+b<1$ 有限型 / $=1$ **仿射型（动态平衡）** / $>1$ 不定型。**"生克总强度 = 1" ＝ "以平为期"。**
